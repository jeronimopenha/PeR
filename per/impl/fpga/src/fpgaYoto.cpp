...
ReportData fpgaYoto(FPGAGraph &g) {
    ...
    for (int i = 0; i < N_DIST_VECTORS; i++) {
        distVectors.push_back(getDistVectors(nCellsSqrt));
    }
    ...
    for (auto [a,b]: ed) {
        ...
        if (runSpiral) {
            ...
            for (const auto &ij: distVectors[distVectorCounter]) {
                ...
                bool isInvalidCell = IsInvalidCell(lB, cB, nCellsSqrt);
                if (isInvalidCell)
                    continue;
                ...
                if (limitStrategyTrigger && !abortLimitStrategy) {
                    ...
                    if (maxValue > 0) {
                        while (true) {
                            ...
                            isTargetCellIO = IsIOCell(lB, cB, nCellsSqrt);
                            isInvalidCell = IsInvalidCell(lB, cB, nCellsSqrt);
                            // Check if the target cell is nor allowed, go to next
...
#ifdef TOTAL_COST
    const long tc = CalcGraphTotalDistance(n2c, g.gEdges, nCellsSqrt);
#elifdef LONG_PATH_COST
    //fixme
    const long tc = 0; //fpgaCalcGraphLPDistance(g.longestPath, n2c, nCellsSqrt);
#elifdef DISTANCE_SLACK_COST
