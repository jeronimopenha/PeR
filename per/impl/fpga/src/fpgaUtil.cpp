 ...
 * @return
 */
string ReportData::to_json() const {
    ostringstream oss;
    oss << "{\n"
            << "  \"time\": " << _time << ",\n"
            << "  \"dotName\": \"" << dotName << "\",\n"
 ...
 * @return
 */
vector<vector<long> > getDistVectors(const long nCellsSqrt) {
 ...
 * @return
 */
bool IsInvalidCell(const long line, const long column, const long nCellsSqrt) {
 ...
 * @return
 */
bool IsIOCell(const long line, const long column, const long nCellsSqrt) {
 ...
 * @return
 */
string ReportData::metrics_to_json() const {
    ostringstream oss;
    oss << "{\n";
    ...
    for (long y = 0; y <= imageCoreSize; y++) {
        for (long x = 0; x <= imageCoreSize; x++) {
            ...
            if (isBorder) {
                ...
                if (isGridLine || IsInvalidCell(srcY, srcX, nCellsSqrt)) {
